...
$(document).ready(function () {
	...
	$(window).bind('resize', function () {
		...
		$("#loading_jdtListSurveyTarget").height(dataTableWrapperHeight);
...
function fun_searchSurveyTarget(){
	var serviceCode = $("#sel_serviceCode").val();
	if(serviceCode==""){
		$("#sel_serviceCode").focus();
		alert("서비스 코드를 선택해주세요");
	...
	$("#loading_jdtListSurveyTarget").show();
	
	var encrypt = fun_encryptMdn(requsetMdn);
	// var startDate = $("#dtp_startDate").val() + " 00:00:00";
	// var closeDate = $("#dtp_closeDate").val() + " 23:59:59";
	
	var inputData = { "secMdn" : encrypt.mdn, "serviceCode": serviceCode, "password":shaPassword, "iv": encrypt.iv}; //, "startDate":startDate, "closeDate":closeDate 
	
	fun_ajaxPostSend("/survey/select/target.do", inputData, true, function(msg){
		if(msg!=null){
			switch(msg.code){
				case "0000":
					var tempResult = JSON.parse(msg.result);
					if(tempResult.length!=0){
						var t = $("#jdtListSurveyTarget").dataTable();
						t.fnClearTable();
						setTimeout(function(){
							fun_dataTableAddData("#jdtListSurveyTarget", tempResult);
							$("#loading_jdtListSurveyTarget").hide();
							$("#txt_adminPassword").val("");
						}, 100);
						setTimeout(function(){
							jdtListSurveyTarget.order([5, 'desc']).draw();
						}, 100);
					}
					else{
						var t = $("#jdtListSurveyTarget").dataTable();
...
function fun_initializeClickEvent(){
	...
	$('#jdtListSurveyTarget').delegate('tr', 'click', function () {
		// 빈테이블이 아닐때
		if (this.innerText != "데이터가 없습니다.") {
			var chk = $(this).hasClass('selected');
			if (chk) {
				...
				$("#jdtListSurveyTarget tbody tr").removeClass('selected');
				...
				this.children[0].lastChild.checked = true;
				//$(this)[0].children(0).lastChild.checked = true;
				fun_setjdtListSurveyTargetToTextbox();   // 선택한 줄 값 라인에 입력
			}
		}
	});
...
function fun_setJdtListMember() {
	jdtListWithoutMember = $("#jdtListWithoutMember").DataTable({
		"columns": [
			...
			, { "title": "제휴사코드", "data": "affiliateCode"}
			, { "title": "통신사코드", "data": "telecomCode"}
			, { "title": "가입시간", "data": "joinDttm"}
			, { "title": "해지시간", "data": "cancelDttm"}
			, { "title": "해지이유", "data": "cancelReason"}
			, { "title": "설문조사 일련번호", "data":"surveySeq"}
		]
		...
		, "columnDefs": [
			{
				"targets": [0]
				, "class": "dt-head-center dt-body-left dtPercent1"
			}
			, {
				"targets": [1]
				, "class": "dt-head-center dt-body-left dtPercent19"
				, "render": function (data, type, row) {
					var tempServiceCode = "";
					for(var i=0; i<allCommonCode.length; i++){
						if(allCommonCode[i].codeGroup == "affiliate_channel_code"){
							if(allCommonCode[i].codeValue == row.affiliateCode){
								tempServiceCode = allCommonCode[i].codeValue + " | " + allCommonCode[i].codeName; 
								break;
							}
						}
					}
				...
				, "render": function (data, type, row) {
					...
					for(var i=0; i<allCommonCode.length; i++){
						if(allCommonCode[i].codeGroup == "telecom_code"){
							if(allCommonCode[i].codeValue == row.telecomCode){
								tempServiceCode = allCommonCode[i].codeValue + " | " + allCommonCode[i].codeName; 
								break;
							}
						}
			...
			, {
				...
				, "class": "dt-head-center dt-body-left dtPercent20"
				, "render": function (data, type, row) {
					if(row.cancelDttm!="가입완료"){
						var regDate = row.cancelDttm;
						regDate = mid(regDate, 0, 4) + "-" + mid(regDate, 4, 2) + "-" + mid(regDate, 6, 2) + " " + mid(regDate, 8, 2) + ":" + mid(regDate, 10, 2) + ":" + mid(regDate, 12, 2);
						return regDate;
					}
					else{
						return row.cancelDttm;
					}
                }
			}
			, {
...
//=== jdt_List Setting
function fun_setjdtListSurveyTarget() {
	jdtListSurveyTarget = $("#jdtListSurveyTarget").DataTable({
		"columns": [
			{ 
				"title": "<input type='checkbox' id='cbx_selectMmsAll' onclick='fun_selectAlljdtListSurveyTarget();'></input>"
				,"data": null
				, "width": "20px"
			...
			, { "title": "제휴사코드", "data": "affiliateCode"}
			, { "title": "통신사코드", "data": "telecomCode"}
			, { "title": "인증번호", "data": "certNumber"}
			, { "title": "인증번호 발송일시", "data": "certDttm"}
			, { "title": "인증번호 실패횟수", "data": "certCount"}
			, { "title": "인증번호 요청횟수", "data": "certRequestCount"}
			, { "title": "설문조사 고객상태", "data": "surveyTargetStatus"}
			, { "title": "설문조사 고객ID", "data": "surveyTargetId"}
			, { "title": "쿠폰상태", "data": "couponStatus"}
			, { "title": "쿠폰결과 코드", "data": "couponReturnCode"}
			, { "title": "쿠폰번호", "data": "couponNumber"}
			, { "title": "쿠폰결과 받은시간", "data": "couponReturnDttm"}
			, { "title": "설문조사 고객상태시간", "data": "surveyTargetStatusDttm"}
			, { "title": "쿠폰 재전송카운트", "data": "couponResendCnt"}
			, { "title": "쿠폰 재전송일시", "data": "couponResendDttm"}
			, { "title": "쿠폰 재전송결과 코드", "data": "couponResendReturnCode"}
		]
		, "paging": false            // Table Paging
		, "info": false             // 'thisPage of allPage'
		, "autoWidth": false
		...
		, "columnDefs": [
			{
				"targets": [0]
				, "class": "dt-head-center dt-body-center dtPercent1"
			}
			, {
				"targets": [1]
			...
			, {
				...
				, "class": "dt-head-center dt-body-left dtPercent5"
				, "render": function (data, type, row) {
					var regDate = row.couponReturnDttm;
					if(regDate==null || regDate ==""){ 
					}
					else{
						regDate = mid(regDate, 0, 4) + "-" + mid(regDate, 4, 2) + "-" + mid(regDate, 6, 2) + " " + mid(regDate, 8, 2) + ":" + mid(regDate, 10, 2) + ":" + mid(regDate, 12, 2);
			...
			, {
				...
				, "class": "dt-head-center dt-body-left dtPercent5"
				, "render": function (data, type, row) {
					var regDate = row.surveyTargetStatusDttm;
					if(regDate==null || regDate ==""){ 
					}
					else{
						regDate = mid(regDate, 0, 4) + "-" + mid(regDate, 4, 2) + "-" + mid(regDate, 6, 2) + " " + mid(regDate, 8, 2) + ":" + mid(regDate, 10, 2) + ":" + mid(regDate, 12, 2);
			...
			, {
				...
				, "class": "dt-head-center dt-body-left dtPercent5"
				, "render": function (data, type, row) {
					var regDate = row.couponResendDttm;
					if(regDate==null || regDate ==""){ 
					}
					else{
						regDate = mid(regDate, 0, 4) + "-" + mid(regDate, 4, 2) + "-" + mid(regDate, 6, 2) + " " + mid(regDate, 8, 2) + ":" + mid(regDate, 10, 2) + ":" + mid(regDate, 12, 2);
...
function fun_setjdtListSurveyTargetToTextbox() {
	var table = $("#jdtListSurveyTarget").DataTable();
...
//=== 전체선택 체크시 이벤트
function fun_selectAlljdtListSurveyTarget() {
	var cbx_selectAll = $("#cbx_selectMmsAll").is(':checked');             // 전체선택 체크시 true
	var tr = $("#jdtListSurveyTarget tbody tr");
	...
	if (tr[0].innerText != "데이터가 없습니다.") {                              // Row에 추가된 값이 있을시
		if (cbx_selectAll) {
			...
			$("#jdtListSurveyTarget tbody tr").removeClass('selected');            // .selected 클래스 제거
		} else {
...
<form id="customer" method="post" onsubmit="return false;">
	<div class="row">
		<div class="col-lg-12">
			<h1 class="page-header" style="font-size:24px">설문조사 레터</h1>
		</div>
		<!-- /.col-lg-12 -->
	</div>
            ...
            <div id="accordion3" class="accordion-style1 panel-group accordion-style2">
                <div class="panel panel-default" id="tog_Head">
				    ...
				    &nbsp;설문조사 참여내역
                        ...
                        <span class="toggle_accordionSearch">
                            <input type="button" id="btn_selectMms" onclick="fun_searchSurveyTarget();" class="btn btn-info btn-sm" value="조회" />
                        </span>
                    </div>
                    <div id="collapse3" class="panel-collapse collapse in">
                        <div class="panel-body">
							...
							<div id="loading_jdtListSurveyTarget" class="loadingDataTable" style="display:none;">
								<img src="../assets/img/loading.gif" alt="loading">
							</div>
                            <div class="dataTable_wrapper" id="dataTable_wrapper1">
                                <table id="jdtListSurveyTarget" class="table table-striped table-bordered table-hover">
                                    <thead>
	...
	<div class="row">
		<div class="col-lg-2">
			<button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#mdl_without" id="btn_withoutToMdl">설문조사레터 해지</button>
		</div>
		<div class="col-lg-2">
			<button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#mdl_coupon" id="btn_couponToMdl" style="opacity: 0;">설문조사 참여내역</button>
		</div>
	</div>

	<div tabindex="-1" class="modal fade" id="mdl_without" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog" style="margin-left: 45%;">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="H5">설문조사레터 해지</h4>
				</div>
				<div class="modal-body">
					...
					<div class="row form-group">
						...
						<div class="col-lg-4">
							<input type="text" id="txt_withoutMdn" class="form-control" placeholder="010-xxxx-xxxx" maxlength="13">
						</div>
					...
					<div class="row form-group">
						...
						<div class="col-lg-6">
							<input type="text" class="form-control" id="txt_cancelReason" maxlength="200" placeholder="해지사유" title="해지사유" disabled>
						</div>
						<div class="col-lg-4">
					...
					<div class="row form-group">
						<div style="color:red">※ 타 서비스는 제공하지 않습니다(설문조사레터만 가능)</div>
					</div>
				</div>
			</div>
		</div>
	...
	<div tabindex="-1" class="modal fade" id="mdl_coupon" role="dialog" aria-labelledby="myCouponLabel" aria-hidden="true">
		<div class="modal-dialog" style="margin-left: 45%;">
			<div class="modal-content">
				<div class="modal-header">
					<button id="mdl_mms_close" class="close" aria-hidden="true" type="button" data-dismiss="modal">×</button>
					<h4 class="modal-title" id="H5">설문조사고객</h4>
				</div>
				<div class="modal-body">
					<div id="loading_without" class="loadingDataTable" style="display:none;margin-left: -15px;margin-top: -15px;">
						<img src="../assets/img/loading.gif" alt="loading">
					...
					<div class="row form-group">
						...
						<div class="col-lg-4">
							<input type="text" id="txt_surveySeq" class="form-control" disabled>
						</div>
						<div class="col-lg-2" style="padding-top:8px;color:red">
							제휴사코드 :
						</div>
						<div class="col-lg-4">
							<input type="text" class="form-control" id="txt_affiliateCode" disabled>
						</div>
					</div>
					<div class="row form-group">
						<div class="col-lg-2" style="padding-top:8px;color:red">
						...
						<div class="col-lg-4">
							<input type="text" id="txt_telecomCode" class="form-control" disabled>
						</div>
						<div class="col-lg-2" style="padding-top:8px;color:red">
							인증번호 :
						</div>
						<div class="col-lg-4">
							<input type="text" class="form-control" id="txt_certNumber" disabled>
						</div>
					</div>
					<div class="row form-group">
						<div class="col-lg-2" style="padding-top:8px;color:red">
						...
						<div class="col-lg-4">
							<input type="text" id="txt_certDttm" class="form-control" disabled>
						</div>
						<div class="col-lg-2" style="padding-top:8px;color:red">
							인증번호 실패횟수 :
						</div>
						<div class="col-lg-4">
							<input type="text" class="form-control" id="txt_certcertCount" disabled>
						</div>
					</div>
					<div class="row form-group">
						<div class="col-lg-2" style="padding-top:8px;color:red">
						...
						<div class="col-lg-4">
							<input type="text" id="txt_certRequestCount" class="form-control" disabled>
						</div>
						<div class="col-lg-2" style="padding-top:8px;color:red">
							설문조사 고객상태 :
						</div>
						<div class="col-lg-4">
							<input type="text" class="form-control" id="txt_surveyTargetStatus" disabled>
						</div>
					</div>
					<div class="row form-group">
						<div class="col-lg-2" style="padding-top:8px;color:red">
						...
						<div class="col-lg-4">
							<input type="text" id="txt_surveyTargetId" class="form-control" disabled>
						</div>
						<div class="col-lg-2" style="padding-top:8px;color:red">
							쿠폰상태 :
						</div>
						<div class="col-lg-4">
							<input type="text" class="form-control" id="txt_couponStatus" disabled>
						</div>
					</div>
					<div class="row form-group">
						<div class="col-lg-2" style="padding-top:8px;color:red">
						...
						<div class="col-lg-4">
							<input type="text" id="txt_couponReturnCode" class="form-control" disabled>
						</div>
						<div class="col-lg-2" style="padding-top:8px;color:red">
							쿠폰번호 :
						</div>
						<div class="col-lg-4">
							<input type="text" class="form-control" id="txt_couponNumber" disabled>
						</div>
					</div>
					<div class="row form-group">
						<div class="col-lg-2" style="padding-top:8px;color:red">
						...
						<div class="col-lg-4">
							<input type="text" id="txt_couponReturnDttm" class="form-control" disabled>
						</div>
						<div class="col-lg-2" style="padding-top:8px;color:red">
							설문조사 고객상태시간 : 
						</div>
						<div class="col-lg-4">
							<input type="text" class="form-control" id="txt_surveyTargetStatusDttm" disabled>
						</div>
					</div>
					<div class="row form-group">
						<div class="col-lg-2" style="padding-top:8px;color:red">
						...
						<div class="col-lg-4">
							<input type="text" id="txt_couponResendCnt" class="form-control" disabled>
						</div>
						<div class="col-lg-2" style="padding-top:8px;color:red">
							쿠폰 재전송일시 :
						</div>
						<div class="col-lg-4">
							<input type="text" class="form-control" id="txt_couponResendDttm" disabled>
						</div>
					</div>
					<div class="row form-group">
						<div class="col-lg-2" style="padding-top:8px;color:red">
						...
						<div class="col-lg-4">
							<input type="text" id="txt_couponResendReturnCode" class="form-control" disabled>
						</div>
						<div class="col-lg-2" style="padding-top:8px;color:red">
							<div id="loading_resendCoupon" class="loadingDataTable" style="display:none;padding:5px 15px 5px 15px!important">
								<img src="../assets/img/loading.gif" alt="loading">
							</div>
							<button type="button" class="btn btn-primary btn-sm" id="btn_resendCoupon" onclick="fun_resendCoupon();">쿠폰 재발행</button>	
						</div>
