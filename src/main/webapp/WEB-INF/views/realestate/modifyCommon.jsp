...
function fun_selectjdtListCommonCode(callback){
	...
	fun_ajaxPostSend("/realestate/select/realestateCommon.do", inputData, true, function(msg){
		if(msg!=null){
			switch(msg.code){
				case "0000":
					break;
...
function fun_clickButtonDeleteCommonCode(){
	...
	if (confirm('삭제 하시겠습니까?')){
		...
		$.ajax({
			type: 'POST'
			, url: '/realestate/delete/realestateCommon.do'
			, async: false
...
function fun_clickButtonUpdateCommonCode(){
	...
	if (confirm('수정 하시겠습니까?')){
		...
		$.ajax({
			type: 'POST'
			, url: '/realestate/update/realestateCommon.do'
			, async: false
			, data: JSON.stringify(inputData)
			, contentType: 'application/json; charset=utf-8'
			, dataType: 'json'
...
function fun_clickButtonInsertCommonCode(){
	...
	if(password==''||password==null){
		...
		$.ajax({
			type: 'POST'
			, url: '/realestate/insert/realestateCommon.do'
			, async: false
			, data: JSON.stringify(inputData)
			, contentType: 'application/json; charset=utf-8'
			, dataType: 'json'
...
<form id="membership" method="post" enctype="multipart/form-data" action="">
   	<div class="row">
	    <div class="col-lg-12">
	        <h1 class="page-header" style="font-size:24px">RealEstate 공통코드 수정</h1>
	    </div>
	    <!-- /.col-lg-12 -->
    </div>
