	...
	Ace_File_Input.prototype.show_file_list = function($files , inner_call) {
		...
		for (var i = 0; i < files.length; i++) {
			...
			if(!this.well_style){
				...
				this.$label.append('<il class="ace-file-name" data-title="'+filename+'"><i class="'+ ace.vars['icon'] + fileIcon+'"></i></il>');
...
(function($ , undefined) {
	$.fn.ace_wysiwyg = function($options , undefined) {
		...
		{
			'font' : {
				values:['Gulim', 'Dotum', 'Batang', 'Gungsuh', 'Malgun Gothic',  'Comic Sans MS', 'Tahoma' ],
				icon:'fa fa-font',
				title:'Font'
			},
			'fontSize' : {
		...
		this.each(function() {
			var toolbar = ' <div id="' + this.id + 'Toolbar" class="wysiwyg-toolbar btn-toolbar center"> <div class="btn-group"> ';
